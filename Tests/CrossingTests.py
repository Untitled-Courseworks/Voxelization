...
class TestsCheckCrossingProjectionAndLine(unittest.TestCase):
    ...
    def test_crossing_zero_point_in_mesh_line(self):
        ...
        res = Cr.check_crossing_projection_and_line([0, 0], 2, [1, 3], [2, 4])
        self.assertFalse(res)

    def test_voxel_between_points(self):
        res = Cr.check_crossing_projection_and_line([0, 0], 2, [1, 3], [1, -2])
        self.assertTrue(res)

    def test_crossing_point_on_vertex_voxel(self):
        res = Cr.check_crossing_projection_and_line([0, 0], 2, [-1, 1], [1, 3])
        self.assertTrue(res)

    def test_line_not_crossing(self):
        res = Cr.check_crossing_projection_and_line([0, 0], 2, [0, 3], [2, 3])
        self.assertFalse(res)

    def test_similarity(self):
        res = Cr.check_crossing_projection_and_line([0, 0], 2, [0, 2], [2, 2])
        self.assertTrue(res)

        res = Cr.check_crossing_projection_and_line([0, 0], 2, [-1, 2], [3, 2])
        self.assertTrue(res)

        res = Cr.check_crossing_projection_and_line([0, 0], 2, [0.5, 2], [1, 2])
        self.assertTrue(res)
